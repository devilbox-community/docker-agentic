		...
		if ! isint "${uid}"; then
			...
			if target_username="$( _get_username_by_uid "${uid}" )"; then
				...
				if [ "${target_username}" != "${username}" ]; then
					...
					run "sudo usermod -u ${spare_uid} ${target_username}" "${debug}"
				fi
			fi
			# Change uid and fix homedir permissions
			log "info" "Changing user '${username}' uid to: ${uid}" "${debug}"
			if ! run "sudo usermod -u ${uid} ${username} 2>/dev/null" "${debug}"; then
				run "sudo usermod -u ${uid} ${username}" "${debug}"
			fi
			run "sudo chown -R ${username} ${homedir} 2>/dev/null || true" "${debug}"
			run "sudo chown -R ${username} /shared 2>/dev/null || true" "${debug}"
		fi
	fi
}
		...
		if ! isint "${gid}"; then
			...
			if target_groupname="$( _get_groupname_by_gid "${gid}" )"; then
				...
				if [ "${target_groupname}" != "${groupname}" ]; then
					...
					run "sudo groupmod -g ${spare_gid} ${target_groupname}" "${debug}"
				fi
			fi
			# Change gid and fix homedir permissions
			log "info" "Changing group '${groupname}' gid to: ${gid}" "${debug}"
			if ! run "sudo groupmod -g ${gid} ${groupname} 2>/dev/null" "${debug}"; then
				run "sudo groupmod -g ${gid} ${groupname}" "${debug}"
			fi
			run "sudo chown -R :${groupname} ${homedir} 2>/dev/null || true" "${debug}"
			run "sudo chown -R :${groupname} /shared 2>/dev/null || true" "${debug}"
		fi
	fi
}
...
if ! command -v sudo >/dev/null 2>&1; then
	log "err" "sudo not found, but required." "1"
	exit 1
fi
