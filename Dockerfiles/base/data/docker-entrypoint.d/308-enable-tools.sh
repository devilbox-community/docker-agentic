...
enable_tools() {
	...
	if ! env_set "${tools_varname}"; then
		log "info" "\$${tools_varname} not set. Not enabling any agentic tools." "${debug}"
	else
		tools="$( env_get "${tools_varname}" )"

		if [ -z "${tools}" ]; then
			log "info" "\$${tools_varname} set, but empty. Not enabling any agentic tools." "${debug}"
			return 0
		fi

		log "info" "Enabling the following agentic tools: ${tools}" "${debug}"
		run "sudo install -d ${bin_dir}" "${debug}"
		...
		while read -r tool; do
			...
			if binary="$( _tool_binary_for_slug "${tool}" )"; then
				target="${prefix}/${tool}/bin/${binary}"
				link="${bin_dir}/${binary}"
				run "sudo ln -sfn ${target} ${link}" "${debug}"
			else
				log "warn" "Enabling agentic tool: '${tool}' does not exist or has no binary" "${debug}"
			fi
		done <<< "$( echo "${tools}" | tr ',' '\n' )"
	fi
}
